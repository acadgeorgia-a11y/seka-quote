...
export function LeadsList({ leads }: { leads: Lead[] }) {
  ...
  const sources = useMemo(() => [...new Set(leads.map(l => l.referral_source).filter(Boolean))].sort(), [leads]);

  const filtered = useMemo(() => leads.filter(l => {
    if (agent  && l.sales_person    !== agent)  return false;
    if (status && l.likelihood      !== status) return false;
    if (source && l.referral_source !== source) return false;
    if (search) {
          ...
          <tbody>
            {filtered.map(l => (
              <tr key={l.id} className="border-b border-border/30 hover:bg-secondary/20 transition-colors">
                ...
                <td className="px-3 py-2.5 whitespace-nowrap">{l.referral_source ?? '—'}</td>
